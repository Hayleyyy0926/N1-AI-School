...
function Field({ label, hint, children, required = true, error, className = '', fieldKey }) {
  return <div className={`field ${error ? 'field-invalid' : ''} ${className}`} data-field={fieldKey}>
...
function App() {
  ...
  const focusFirstError = (fieldKey, sectionId) => {
    const focus = () => {
      const wrapper = fieldKey ? document.querySelector(`[data-field="${fieldKey}"]`) : null
      const control = wrapper?.querySelector('input:not([type="hidden"]), textarea, button')
  ...
  const showValidation = validation => {
    ...
    focusFirstError(validation.firstField, validation.firstSection)
  }
  ...
  const goNext = async () => {
    ...
    try {
      ...
      if (error.details?.errors) {
        showValidation({ errors: error.details.errors, firstField: error.details.firstField, firstSection: error.details.firstSection })
      } else {
  ...
  const firstErrorLabel = Object.keys(errors).map(key => FIELD_LABELS[lang][key]).find(Boolean)
    ...
    <div className="layout">
      <aside className="side-nav">
        ...
        <nav aria-label="Form sections">
          {sections.map(section => <button key={section.id} className={`nav-item ${section.id === active ? 'current' : ''}`} onClick={() => setActive(section.id)}>
            <span className="nav-no">{section.no}</span><span className="nav-name"><span>{isZh ? section.zh : section.en}</span></span><span className="nav-arrow">↗</span>
          </button>)}
        </nav>
      ...
      <main className="form-main">
        {submitted ? <div className="success-panel"><span className="success-mark">✓</span><span className="eyebrow">APPLICATION RECEIVED</span><h2>{isZh ? '申请已提交' : 'Application submitted'}</h2><p>{isZh ? '感谢你认真完成这份申请。我们会在下一轮联系你。' : 'Thank you for taking the time to apply. We will be in touch about the next stage.'}</p><button className="next-btn" onClick={() => setSubmitted(false)}>{isZh ? '返回查看申请' : 'Review application'} <span className="next-icon">→</span></button></div> : <>
          ...
          {submitAttempted && Object.keys(errors).length > 0 && <div className="validation-summary" role="alert" aria-live="assertive"><strong>{isZh ? '请完成此必填问题后再提交' : 'Complete this required question before submitting'}</strong>{firstErrorLabel && <p>{firstErrorLabel}</p>}</div>}
...
function Instructions({ isZh }) {
  return <div className="section-body instruction-body">
    <SectionIntro no="00" title={isZh ? '填写说明' : 'Before You Begin'} />
    {isZh ? <>
...
function Basics({ language, form, set, errors }) {
  ...
  return <div className="section-body"><SectionIntro no="一" title={isZh ? '基本信息' : 'Basic Information'} />
    ...
    <div className="rule" />
    <Question no="02" title={isZh ? '目前状态' : 'Current Status'}>
    ...
    <Field fieldKey="status" label={isZh ? '目前状态' : 'Current status'} error={errors.status}><div className="pill-grid">{STATUS_OPTIONS.map(option => <button type="button" key={option.value} className={form.status === option.value ? 'selected' : ''} onClick={() => set('status', option.value)}>{option[language]}</button>)}</div></Field>
    <Field fieldKey="school" label={isZh ? '学校、专业、年级或当前正在做的事情' : 'School, major, year, or what you are currently working on'} error={errors.school}><input aria-label={isZh ? '学校、专业、年级或当前正在做的事情' : 'School, major, year, or current work'} value={form.school} onChange={event => set('school', event.target.value)} /></Field>
  </div>
}

function CoreOne({ language, form, set, errors }) {
  const isZh = language === 'zh'
  return <div className="section-body"><SectionIntro no="二" title={isZh ? '核心问题' : 'Core Questions'} />
    <Question no="03" title={isZh ? '请告诉我们你做过最重要的一件事' : 'Tell us about the most important thing you have done'}>
    ...
    <Field fieldKey="mostImportantWork" label={isZh ? '回答' : 'Answer'} error={errors.mostImportantWork}><TextArea label={isZh ? '第 3 题回答' : 'Question 3 answer'} value={form.mostImportantWork} onChange={event => set('mostImportantWork', event.target.value)} limit={TEXT_LIMITS.mostImportantWork} language={language} /></Field>
    <div className="rule" />
    <Question no="04" title={isZh ? '【选填】如果还有一件完全不同的作品能帮助我们理解你，可以提交' : 'Optional: Share another substantially different piece of work'}><p>{isZh ? '最多 200 字，可附链接或证据。' : 'Maximum 200 words. Links or evidence may be included.'}</p></Question>
    <Field fieldKey="optionalDifferentWork" label={isZh ? '回答' : 'Answer'} required={false} error={errors.optionalDifferentWork}><TextArea label={isZh ? '第 4 题回答' : 'Question 4 answer'} value={form.optionalDifferentWork} onChange={event => set('optionalDifferentWork', event.target.value)} limit={TEXT_LIMITS.optionalDifferentWork} language={language} /></Field>
    <div className="rule" />
    <Question no="05" title={isZh ? '过去一年，你靠自己学会的最难的一件事是什么？' : 'What is the hardest thing you taught yourself in the past year?'}>
    ...
    <Field fieldKey="selfTaughtChallenge" label={isZh ? '回答' : 'Answer'} error={errors.selfTaughtChallenge}><TextArea label={isZh ? '第 5 题回答' : 'Question 5 answer'} value={form.selfTaughtChallenge} onChange={event => set('selfTaughtChallenge', event.target.value)} limit={TEXT_LIMITS.selfTaughtChallenge} language={language} /></Field>
    <div className="rule" />
    <Question no="06" title={isZh ? '什么事情会让你忘记时间？' : 'What makes you lose track of time?'}>
    ...
    <Field fieldKey="loseTrackOfTime" label={isZh ? '回答' : 'Answer'} error={errors.loseTrackOfTime}><TextArea label={isZh ? '第 6 题回答' : 'Question 6 answer'} value={form.loseTrackOfTime} onChange={event => set('loseTrackOfTime', event.target.value)} limit={TEXT_LIMITS.loseTrackOfTime} language={language} /></Field>
  </div>
}
...
function CoreTwo({ language, form, set, errors, onUpload, uploading, uploadError }) {
  const isZh = language === 'zh'
  return <div className="section-body"><SectionIntro no="二" title={isZh ? '核心问题' : 'Core Questions'} />
    <Question no="07" title={isZh ? '有什么事情是你相信的，但身边很多聪明的人不同意？' : 'What is something you believe that many smart people around you disagree with?'}>
    ...
    <Field fieldKey="disagreement" label={isZh ? '回答' : 'Answer'} error={errors.disagreement}><TextArea label={isZh ? '第 7 题回答' : 'Question 7 answer'} value={form.disagreement} onChange={event => set('disagreement', event.target.value)} limit={TEXT_LIMITS.disagreement} language={language} /></Field>
    <div className="rule" />
    <Question no="08" title={isZh ? '如果接下来一个月完全由你自己决定，你最想把什么做出来或弄明白？' : 'If you could decide everything for the next month, what would you most want to build or figure out?'}>
    ...
    <Field fieldKey="nextMonth" label={isZh ? '回答' : 'Answer'} error={errors.nextMonth}><TextArea label={isZh ? '第 8 题回答' : 'Question 8 answer'} value={form.nextMonth} onChange={event => set('nextMonth', event.target.value)} limit={TEXT_LIMITS.nextMonth} language={language} /></Field>
    <div className="rule" />
    <Question no="09" title={isZh ? '给我们看一点你做事的过程' : 'Show us a little of your process'}>
    ...
    <Field fieldKey="processEvidence" label={isZh ? '链接' : 'Link'} error={errors.processEvidence && !form.processFile?.fileToken ? errors.processEvidence : ''}><input aria-label={isZh ? '第 9 题链接' : 'Question 9 link'} type="url" value={form.process} onChange={event => set('process', event.target.value)} placeholder="https://" /></Field>
    <div className="rule" />
...
function Video({ language, form, set, errors, onUpload, uploading, uploadError }) {
  const isZh = language === 'zh'
  return <div className="section-body"><SectionIntro no="三" title={isZh ? '视频' : 'Video'} />
    <Question no="11" title={isZh ? '两分钟，不剪辑，不念稿' : 'Two minutes, unedited, no script'}>
    ...
    <Field fieldKey="videoEvidence" label={isZh ? '视频链接' : 'Video link'} error={errors.videoEvidence && !form.videoFile?.fileToken ? errors.videoEvidence : ''}><input aria-label={isZh ? '第 11 题视频链接' : 'Question 11 video link'} type="url" value={form.video} onChange={event => set('video', event.target.value)} placeholder="https://" /></Field>
  </div>
}

function Confirmation({ language, form, set, toggle, errors }) {
  const isZh = language === 'zh'
  return <div className="section-body"><SectionIntro no="—" title={isZh ? 'AI 使用说明' : 'AI Use'} />
    <Question no="AI" title={isZh ? '你在完成申请时如何使用了 AI？' : 'How did you use AI while completing this application?'}>
    ...
    <Field fieldKey="ai" label={isZh ? 'AI 使用方式' : 'AI use'} error={errors.ai}><OptionList options={AI_OPTIONS} values={form.ai} onToggle={value => toggle('ai', value)} language={language} /></Field>
    <Field fieldKey="aiNote" label={isZh ? '请简要说明' : 'Please explain briefly'} error={errors.aiNote}><textarea aria-label={isZh ? 'AI 使用说明' : 'AI use explanation'} value={form.aiNote} onChange={event => set('aiNote', event.target.value)} /></Field>
    <div className="rule" />
    <SectionIntro no="—" title={isZh ? '申请人确认' : 'Applicant Confirmation'} />
